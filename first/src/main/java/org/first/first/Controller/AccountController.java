...
import org.first.first.services.EmailService;
import org.first.first.util.AppUtill;
import org.first.first.util.email.EmailDetails;
...
public class AccountController {
    ...
    public String update_photo(@RequestParam("file") MultipartFile file, RedirectAttributes attributes, Principal principal){
        if(file.isEmpty()){
            ...
            try {
                ...
                String absolutefileLocation = AppUtill.get_upload_path(final_photo_name);
                ...
                if(optionalAccount.isPresent()){
                    Account account = optionalAccount.get();
                    Account account_by_id = accountService.findById(account.getId()).get();
                    String relativeFileLocation = "/uploads/" + final_photo_name;
    ...
    public String reset_passwprd(@RequestParam("email") String _email, RedirectAttributes attributes, Model model){
        ...
        if(optionalAccount.isPresent()){
            ...
            EmailDetails emailDetails =new EmailDetails(account.getEmail(), reset_message, "reset password InkBliss");
